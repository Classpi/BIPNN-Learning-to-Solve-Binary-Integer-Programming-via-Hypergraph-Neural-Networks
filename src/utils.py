...
# QUBO utils
def generate_Q(n, zero_ratio, seed=0):
    """Generate a ramdom Q matrix. Q matrix is a symmetric matrix.

    Args:
        n (int): Matrix dimension.
        zero_ratio (float): Ratio of zeros in matrix.
    ...
    matrix = 2 * torch.rand(n, n) - 1.0
    total_elements = n * n
    zero_elements = int(total_elements * zero_ratio)
    zero_indices = torch.randperm(total_elements)[:zero_elements]
    matrix.view(-1)[zero_indices] = 0
    symmetric_matrix = torch.triu(matrix) + torch.triu(matrix, diagonal=1).t()

    return symmetric_matrix


def generate_graph(Q):
    """Generate a graph from the Q matrix.
    ...
    Returns:
        Graph: graph.
    """
    nonzero_indices = torch.nonzero(deepcopy(Q).fill_diagonal_(0))
    edgelist = [tuple(index.tolist()) for index in nonzero_indices]
    return dhg.Graph(Q.shape[0], edgelist)


# PUBO utils
def generate_hypergraph(k: int, n: int, e: int, seed=5):
    """Generate H matrix.
...
def generate_hypergraph_Gnm(n, e, seed=0):
    """
    Generating a general hypergraph with `n` vertices and `e` hyperedges

    Notes:
        Since the hypergraph generated by DHG does not have hyperedges with a single vertex,
        we need to add it manually.
    """

    def add_single_v_hyperedges(hypergraph: dhg.Hypergraph) -> None:
        for v in hypergraph.v:
            hypergraph.add_hyperedges([v])

    dhg.random.set_seed(seed)
    hg = dhg.random.hypergraph_Gnm(n, e)
    add_single_v_hyperedges(hg)
    return hg
...
def from_file_to_hypergraph(file_path: str, reset_vertex_index: bool = False) -> Hypergraph:
    ...
    with open(file_path, "r") as file:
        ...
        return Hypergraph(
            num_v=len(all_vertices),
            e_list=processed_edges
        )
...
def get_obj_value(H, C, outs):
    H_expand = H.unsqueeze(1)
    outs_expanded = outs.unsqueeze(2)
    H0 = outs_expanded * H_expand
    H1 = torch.where(H_expand == 0, torch.tensor(1.0, device=H.device), H0)
    H2 = torch.prod(H1, dim=0)
    H2_expand = H2.unsqueeze(1)
    obj_per_group = torch.mul(H2_expand, C)
    obj = obj_per_group.sum(dim=-1)
    return obj



def _gumbel_sample(logits: torch.Tensor, N: int, tau: float = 1.0):
    logits = logits.reshape(-1, 1)
    logits = logits.repeat(N, 1, 1)
    logits = torch.cat([torch.zeros_like(logits), logits], dim=-1)
    return torch.nn.functional.gumbel_softmax(logits, tau=tau, hard=True)[:, :,
                                                                          1]